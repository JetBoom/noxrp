...
function CreateBullet(pos, dir, attacker, inflictor, damage, speed, class, fromweapon, nocompensation, customfilter)
	--[[if fromweapon then
	...
	local tr = BulletTrace(pos, pos + math.max(0.04, pingmul) * vel, customfilter or (not customfilter and attacker and attacker:GetAttackFilter()))
	if tr.Hit then
		prehit = tr
	end
	...
	if bullet:IsValid() then
		//Check this here since we need to have the bullet entity to call this
		if fromweapon then
			inflictor:CheckOnCreatedBullet(bullet)
		end
		...
		if attacker and attacker:IsValid() then
			...
			if attacker:IsPlayer() then
				...
				bullet:SetColor(Color(200, 200, 200, 255))
			end
		end
		...
		bullet.DoSkillCheck = attacker
		
	//	if customfilter then
	//		bullet.BulletFilter = customfilter
	//	end
