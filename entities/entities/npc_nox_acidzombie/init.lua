...
function ENT:Swing()
	...
	for _, filt in pairs(ents.FindByClass(self:GetClass())) do
		if filt:GetPos():Distance(self:GetPos()) <= 80 then
	...
	while ( path:IsValid() ) do
		...
		if target:IsValid() then
			...
			if self:GetPos():Distance(target:GetPos()) <= self.StopMovingDist then
				...
				return "ok"
			elseif self.NextAcid < CurTime() and self:GetPos():Distance(target:GetPos()) <= 800 then
				local data = {}
					data.start = self:GetPos() + self:OBBCenter()
					data.endpos = target:GetPos() + target:OBBCenter()
					data.filter = self
...
function ENT:RunBehaviour()
    while ( true ) do
		if self:GetTarget():IsValid() then
			local target = self:GetTarget()
			if self.State ~= STATE_ATTACKING and self.State ~= STATE_SWUNG and self.State ~= STATE_ACIDTHROW then
				local dist = self:GetPos():Distance(target:GetPos())
				--they're too far, so start walking
				
				if dist <= 800 and self.NextAcid < CurTime() then
					if self:IsFacing(target) then
						self:StartActivity( ACT_MELEE_ATTACK1 )
						self.State = STATE_ACIDTHROW
						self.SwingTime = CurTime() + 0.4
						self.NextAcid = CurTime() + 5
					else
						self.loco:FaceTowards(target:GetPos() + target:OBBCenter())
					end
				elseif dist > self.StopMovingDist then
				...
				elseif self.State == STATE_ACIDTHROW then
					if self.SwingTime < CurTime() then
						self.State = STATE_IDLE
						
						local acid = ents.Create("projectile_acidball")
						if acid:IsValid() then
							acid:SetPos(self:GetPos() + self:OBBCenter() + Vector(0, 0, 10) + self:GetForward() * 30)
							acid:Spawn()
							acid:Launch(self:GetForward(), 1000)
						end
					else
						if not self:IsFacing(target) then
							self.loco:FaceTowards(target:GetPos() + target:OBBCenter())
